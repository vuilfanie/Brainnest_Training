...
let draw = playerSelection === computerSelection;
  const result = /rockscissors|scissorspaper|paperrock/gi.test(playerSelection + computerSelection);

  return draw ? "Draw!" : `${result ? "Your Point!" : "Computer Point!"}, ${result ? playerSelection : computerSelection} beats ${result ? computerSelection : playerSelection}.`;
};
...
if (roundNumber === 5) {
  ...
  alert(`${userScore > computerScore 
      ? "You won the game!" : userScore < computerScore 
      ? "The computer won the game!" : "It's a draw!"} 
